...
WELCOME_MESSAGE = "Unleash your inner adventurer and let's explore Oklahoma. " \
    " On your adventure, you start with " \
     "a lot of Points. " \
    "But the choices you make will either increase or decrease your Points. " \
    "Your adventure ends when you run out of Points. " \
    "Try to stay on your adventure as long as you can! To get help from guide, say give me tip!! " \
    "Begin your adventure by saying play or start"

VISIT_COUNTRY_REPROMPT = "Say start or play to begin your adventure?"
YES_OR_N0_REPROMPTS = ['Do not stall adventurer! Please answer yes or no. If you need a tip, say speak to the guide.','Be careful adventurer, is your answer yes or no.','You are running out of time adventurer! Please answer yes or no.','Adventurer, is your answer yes or no. If you need a tip, say speak to the guide.','Yes or No, adventurer! If you need a tip, say speak to the guide.']
GAME_END = "The next question could not be found for your adventure. You have reached the end. Hope you had fun playing this game."
...
class StartAdventureIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        try:
            ...
            if is_user_on_session(handler_input):
                ...
                if is_returning_user(handler_input):
                    ...
                    if has_active_adventure(handler_input):
                        ...
                        #play audio via SSML
                        speak_output = "Welcome to your new adventure!" + speak_output 
                        reprompt_output = YES_OR_N0_REPROMPTS[randint(0, len(YES_OR_N0_REPROMPTS)-1)]  
                else:
                    add_new_user(handler_input.request_envelope.context.system)
                    #ask if they want 
            ...
            speak_output = "Sorry, adventurer! I don't understand what you want to do. That feature is probably not supported yet. {}".format(VISIT_COUNTRY_REPROMPT)
            reprompt_output = VISIT_COUNTRY_REPROMPT
    ...
    def handle(self, handler_input):
        ...
        try:
            ...
            if is_user_entitled(isp_response):
                ...
                tip_for_question = get_tip_for_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"], handler_input)
                next_question = get_next_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"],handler_input)
                speak_output = "<voice name=\""+ get_polly_voice(handler_input.attributes_manager.session_attributes["country"]) + "\">Here is your tip from your guide!! " + tip_for_question + "</voice> " + next_question  
                reprompt_output = "<voice name=\""+ get_polly_voice(handler_input.attributes_manager.session_attributes["country"]) + "\"> Adventurer, don't hesistate! " + tip_for_question + "</voice> " + next_question 
                include_display(handler_input)
            ...
            speak_output = "A tip is not available at this time. Make sure that you are in active game play. Say play or start to begin the game."
            reprompt_output = "Would you like to start the game?"
            return (
                response_builder
                    .speak(speak_output)
                    .ask(reprompt_output) 
        ...
        if handler_input.request_envelope.request.status.code == "200":
            if is_returning_user(handler_input) and has_active_adventure(handler_input):
                if handler_input.request_envelope.request.payload.get("purchaseResult") == PurchaseResult.DECLINED.value:
                    ...
                    get_next_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"],handler_input)))
                    ...
                    speech = ("Tip from your guide is: {}. {}".format(get_tip_for_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"], handler_input),
                            get_next_question(handler_input.attributes_manager.session_attributes["country"], handler_input.attributes_manager.session_attributes["stats_record"],handler_input)))
                    reprompt = YES_OR_N0_REPROMPTS[randint(0, len(YES_OR_N0_REPROMPTS)-1)]  
                    return response_builder.speak(speech).ask(reprompt).response
        else:
...
class HelpIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speak_output = "Hello, adventurer! It's good to see you! To begin this game say play or start. If you want tips from guide, say speak to the guide. Don't forget that your wealth either increase or decreases based on the choices you make while on your adventure. When you run out of wealth, the game ends. " 
...
class CancelOrStopIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speak_output = "Goodbye adventurer!" + getRandomFact() + ". We will be adding more categories like sports, landmarks, science, arts soon!"
        response_builder = handler_input.response_builder
        include_display(handler_input)
...
class FallbackIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        ...
        speech = (
                ...
                "continue on your adventure by saying play or start to begin the game. "
            )
...
def getRandomFact():
    ...
    if fact_record['Count'] == 1:
        ...
        return "Oklahoma is known for tornados but do you know Oklahoma is also getting lots of minor earthquakes"
...
def getYesorNoResponse(handler_input, textType):
    ...
    try: 
        ...
        if question_record['Count'] == 1: 
            ...
            speak_output = question_record['Items'][0][textType] + '  ' + question_record['Items'][0]['ResponseText']
            ...
            if is_game_over(handler_input.attributes_manager.session_attributes["stats_record"]):
                speak_output = "<voice name=\""+ get_polly_voice(handler_input.attributes_manager.session_attributes["country"]) + "\">Oh no adventurer, you don't have enough wealth to continue on your adventure! This means your adventure is over. </voice> " 
                ...
                speak_output = "<voice name=\""+ get_polly_voice(handler_input.attributes_manager.session_attributes["country"]) + "\">Be careful adventurer, you are running low on wealth. If you need a tips from guide, say speak to the guide.</voice> " 
...
def get_country_name(CountryId):
    ...
    if country_record['Count'] == 1:
        ...
        logger.error("Cannot find country name (aka category) for given id {}".format(CountryId)) 
        raise AskSdkException("Cannot find country name (aka category) for given id {}".format(CountryId)) 
...
def start_new_adventure(handler_input):
    ...
    handler_input.attributes_manager.session_attributes["country"] = 'History'

    new_adventure = {
        "PlayerNumber": handler_input.attributes_manager.session_attributes["user"]['Items'][0]['PlayerNumber'],
        "QuestionNumber": 0,
        "CountryId": '1', #commented by Michael Agarwal get_country_id(handler_input.request_envelope.request.intent.slots['country'].value),
        "CurrentTurns": 0,
        "MoneyLevel": 50,
        "EnergyLevel": 50,
        "ActiveFlag" : 'Y',
...
def is_warning_needed(current_wealth,current_energy):
    if current_wealth <= 10 or current_energy <= 0:
        return True
    else:
        return False
...
def get_tip_for_question(countryname, stats, handler_input):
    table = boto3.resource('dynamodb').Table('AdvgStoryDetails')
    speak_output = GAME_END
...
# Secure the skill by setting the skill id 
# Uses incorrect skill id to demonstrate rejection
#sb.skill_id = "amzn.ask.skill.12345"
